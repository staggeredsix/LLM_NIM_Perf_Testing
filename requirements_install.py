...
def install_nvidia_toolkit():
    ...
    run_command("distribution=$(. /etc/os-release;echo $ID$VERSION_ID)", "Failed to set distribution variable")
    run_command("curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey | sudo apt-key add -", "Failed to add NVIDIA Docker GPG key")
    run_command("curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list | sudo tee /etc/apt/sources.list.d/nvidia-docker.list", "Failed to add NVIDIA Docker repository")
...
def install_ngc_cli():
    ...
    run_command("wget -O ngccli_cat_linux.zip https://ngc.nvidia.com/downloads/ngccli_cat_linux.zip && unzip ngccli_cat_linux.zip && chmod u+x ngc", "Failed to download or install NVIDIA NGC CLI")
...
def check_python():
    """Check if Python 3 is installed and prompt to install if not."""
    if not sys.version_info >= (3, 6):
        response = input("Python 3 is not installed. Do you want to install it? (y/n): ").strip().lower()
        if response == 'y':
            run_command("sudo apt-get install -y python3 python3-pip", "Failed to install Python 3")
        else:
